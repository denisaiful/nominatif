...
$xAxis = 800; //$_POST['xAxis'];
$yAxis = 200; //$_POST['yAxis'];  
$width = 3; //$_POST['width'];  
$height = 270; //$_POST['height'];          
...
//echo "<script>alert('".$status_code."'); </script>";
	 ...
	 if ($status_code_api == 200 ) {
		 ...
		 //echo "<script>alert('".$status_code."'); </script>";
			$link = mysqli_connect('localhost', 'webadmin', 'pusdiklatNOMINATIF@&)&@@4', 'sertifikat');
			$update = mysqli_query($link,"update spt_peserta SET status_ttd = 1,file_ttd = '$filename_ttd' WHERE no_peserta=$Id_peserta AND `status_ttd` = 0"); 
		...
		$input = mysqli_query($link,"INSERT INTO `sertifikat`.`log_esign` (
			...
			echo "<script>alert('SPT berhasil Ditandatangani');
 			window.location='../index2a.php?data=tables2';</script>"; 
...
$input = mysqli_query($link,"INSERT INTO `diklatdb2`.`log_esign` (
    ...
    $no++; 
		...
		$count_tes++;
		echo "<script>alert('Blom berhasil".$count_tes."');
 			window.location='../index2a.php?data=tables2';</script>";  
}
...
//$content = '<page  backimg="../img/surat.png">'.nl2br($content).'</page>';
//include_once("koneksi.php"); //buat koneksi ke database
$link = mysqli_connect('localhost', 'webadmin', 'pusdiklatNOMINATIF@&)&@@4', 'sertifikat');
$kode   = $getKegiatan; //$_GET['getKegiatan']; //kode berita yang akan dikonvert
$query  = mysqli_query($link, "SELECT * FROM spt_baru WHERE no_peserta='".$kode."' AND `status_ttd` = 0  ");
...
<p>Demikian, untuk dilaksanakan sebagaimana mestinya.</p>
  <table>
   <tr>
    ...
    <td width="300" align="left" valign="top"> Jakarta, '.$data['tanggal'].' '.$data['bulan'].' '.$data['tahun'].'<br> '.$data['status'].',</td>
  </tr>
   <tr>
    <td width="250" ></td>  
 ...
 <table border="1"> 
   <tr >
   <td width="1000" align="left" style="font-size:8pt; color: #003E75;" >Dokumen ini ditandatangani secara elektronik menggunakan Sertifikat 
Elektronik yang diterbitkan oleh Balai Sertifikasi Elektronik (BSrE),<br>
 Badan Siber dan Sandi Negara (BSSN)</td>
...
// $filename="Nominatif ".$kode.".pdf"; 
	...
	$format =array('215.9','310');
	$html2pdf = new Html2Pdf('P',$format,'fr', true, 'UTF-8', array(30, 45, 30, 0), false); 
