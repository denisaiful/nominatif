...
$selisih=$jumlah['banyak'];
	...
	<div class="c">
	Kode :3342.'.$r['akun2'].'.'.$r['akun'].'.'.$r['akun1'].'.'.$r['kom'].'.'.$r['sub_kom'].'.'.$r['mak'].'
	</div>	
...
substr(`kuitansi_baru`.`tgl_spt_new`,1,4) AS `tahun_spt`
		...
		from kuitansi_baru  WHERE no_keg=$kode LIMIT 33 ");
        $no 	= 1;
        while ($r = mysqli_fetch_array($sql)) { 
		$bilangan=$r['no_urut'];
...
substr(`kuitansi_baru`.`tgl_spt_new`,1,4) AS `tahun_spt`
		...
		from kuitansi_baru  WHERE no_keg=$kode LIMIT 33,70 ");
        $no 	= 34;
        while ($r = mysqli_fetch_array($sql)) { 
		$bilangan=$r['no_urut'];
$no_urut = sprintf("%05d", $bilangan);
...
ppk.name AS name_ppk,
bendahara.name AS name_bendahara,
bendahara.id AS id_bendahara,
kortim.`name`,
...
FROM
spt_peserta
LEFT JOIN spt_pimpinan ON spt_pimpinan.`no` = spt_peserta.kegiatan_peserta
LEFT JOIN kortim ON kortim.id = spt_pimpinan.kortim
LEFT JOIN tim_kerja ON tim_kerja.id = kortim.id_tim
LEFT JOIN ppk ON spt_pimpinan.ppk = ppk.id
LEFT JOIN bendahara ON spt_pimpinan.bendahara = bendahara.id
WHERE
...
$query  = mysqli_query($koneksi,"select * from kuitansi WHERE no_keg='".$kode."'");
$data   = mysqli_fetch_array($query);  
$keg = $data['no_keg'];
$filename="Nominatif ".$keg.".pdf"; 
require __DIR__.'/html2pdf_v5.2-master/vendor/autoload.php';
