...
$sql 	= mysqli_query($link,"select * from kuitansi_baru WHERE no_peserta=$kode");
...
$query  = mysqli_query($link,"select * From kuitansi_baru  WHERE no_peserta='".$kode."'");
...
$content .= '  
  ...
  $query  = mysqli_query($link,"select * From kuitansi_baru  WHERE no_peserta='".$kode."'");
$data   = mysqli_fetch_array($query);
    $transport_lokal =$data['transport_lokal'];
...
$content .= '  
  ...
  <td style="border-bottom:0px;">- Biaya Transport Lokal</td>
  <td style="border-bottom:0px;border-right:0px;">Rp.</td>
  <td align="right" width="80"style="border-bottom:0px;">'. number_format($data['transport_lokal'],0, ',','.').'</td>
  <td style="border-bottom:0px;"></td>
</tr>
   ';
  };
...
<tr>  
  <td style="border-bottom:0px;">'.$data['hari_fullboad'].' hari Fullboard x Rp. '. number_format($data['fullboard'],0, ',','.').' </td>
  <td style="border-right:0px;border-bottom:0px;">Rp.</td>
...
$query  = mysqli_query($link,"select * From kuitansi_new  WHERE no_peserta='".$kode."'");
$data   = mysqli_fetch_array($query);
    $fullboard =$data['fullboard'];
    if ($fullboard==0)
      { $content .= '
...
$data   = mysqli_fetch_array($query);
    ...
    if ($date>$date2)
      { $content .= '
  <tr>
    <td align="center" ><u>'.$data['name_bendahara'].'</u> <br>NIP. '.$data['nip_bendahara'].'</td>
    <td width="170" ></td>
...
$data   = mysqli_fetch_array($query);
    ...
    <td width="300" height="80" align="center" valign="bottom"><u>'.$data['name_ppk'].'</u><br> NIP.'.$data['nip_ppk'].'</td>
   ';}elseif ($date<$date2){
$content .= '
...
<table>
  ...
  <table style="border-bottom:1px";>
   <tr>
    <td align="center" width="700" ><strong><h3>TRANSPORT LOKAL</h3></strong></td>
  </tr>
  </table>
  <br><br>
  <table width="700">
...
$data   = mysqli_fetch_array($query);
    ...
    <td height="80" valign="bottom"><u>'.$data['name_ppk'].'</u> <br>NIP.'.$data['nip_ppk'].'</td>
	 ';}elseif ($date<$date2){
$content .= '
	<td height="80" valign="bottom"><u>'.$data['name_ppk'].'</u><br> NIP.'.$data['nip_ppk'].'</td>
';}
...
$data   = mysqli_fetch_array($query);
    ...
    <td ></td>
    <td colspan="3" align="center" width="300" height="50" valign="bottom"><u>'.$data['name_ppk'].'</u></td>
  </tr>
  <tr>
    <td ></td>
    <td colspan="3" align="center">NIP.'.$data['nip_ppk'].'
	</td>
	 ';}elseif ($date<$date2){
$content .= '
...
$data   = mysqli_fetch_array($query);
$nama =$data['nama'];
$filename="kuitansi ".$kode."-".$nama.".pdf"; 
require __DIR__.'/html2pdf_v5.2-master/vendor/autoload.php';
