...
//view
if(isset($_GET['idx'])) {
	$view = mysql_query("select * from spt_peserta INNER JOIN spt_pimpinan
	...
	<label >Ganti Kegiatan</label>	
     ...
     <select id="kota" name="kegiatan_peserta" class="form-control">
                    <option value="<?php echo $dt_view['kegiatan_peserta'] ?>"><?php echo $dt_view['kegiatan'] ?></option>
                            <?php 
	include "koneksi.php";  	   
    $result = mysql_query("select * from spt_pimpinan_new order by no desc");	
...
// test ajy
$no_peserta = mysql_query("SELECT kegiatan_peserta FROM spt_peserta WHERE no_peserta=$idx ");
...
if(isset($_POST['update'])) {
	...
	$update = mysql_query("UPDATE `sertifikat`.`spt_peserta` SET nama_peserta ='$nama_peserta', 
...
//delete
if(isset($_GET['d'])) {
	$del = mysql_query("delete from nomor where id=$idx");
	
	if($del>0){echo "<script>alert('data berhasil dihapus');
 			window.location='index.php';</script>";}else{ echo "hapus data gagal";}
