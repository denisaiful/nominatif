...
<script type="text/javascript">
    ...
    function drawChart() {
	  ...
	  <?php
					...
					$bulan = date("m");
					$tahun = date("Y");  
                    $sql = mysqli_query($koneksi,"SELECT nama_peserta,nip_peserta,sum(lama) AS lama, bulan1, tanggal_pergi, tanggal_pulang,bulan_pergi,bulan_pulang,tahun_new, COUNT(nip_peserta) AS jlh FROM `data_spt2` WHERE ( 
					...
					) AND tahun_new = '$tahun' AND bulan_pergi = 8 AND bulan_pulang = 8 GROUP BY nip_peserta ORDER BY `data_spt2`.`nama_peserta` ASC");
                    $no = 1;
                    while ($r = mysqli_fetch_array($sql)) {
						
                    
	  
        echo "['".$r['nip_peserta']."', '".$r['nama_peserta']."', 'spring',
         new Date(".$r['tahun_new'].", ".$r['bulan_pergi'].", ".$r['tanggal_pergi']."), new Date(".$r['tahun_new'].", ".$r['bulan_pulang'].", ".$r['tanggal_pulang']."), null, 100, ".'null'.", ],";
